...
function isAnswer(value) {
  return ANSWERS.has(String(value || "").trim().toUpperCase());
}

async function startQuiz(db, userId, category, difficulty) {
  const normalizedCategory = normalizeCategory(category);
  const normalizedDifficulty = normalizeDifficulty(difficulty);
  if (!normalizedCategory) throw Object.assign(new Error("Invalid category."), { code: "VALIDATION" });
  if (difficulty && !normalizedDifficulty) throw Object.assign(new Error("Invalid difficulty."), { code: "VALIDATION" });
  ...
  async function sampleQuestions(comparison, point, sourceFilter = filter, sourceParams = baseParams) {
  ...
  if (questions.length < QUESTION_LIMIT) {
    ...
    const fallbackParams = normalizedDifficulty ? [normalizedCategory] : baseParams;
    const additional = await sampleQuestions("<", randomPoint, fallbackFilter, fallbackParams);
  ...
  if (!questions.length) throw Object.assign(new Error("No questions are available for this selection."), { code: "NOT_FOUND" });
...
async function answerQuestion(db, userId, attemptId, questionId, answer) {
  if (!Number.isInteger(attemptId) || attemptId <= 0 || !Number.isInteger(questionId) || questionId <= 0 || !isAnswer(answer)) {
    throw Object.assign(new Error("Invalid answer request."), { code: "VALIDATION" });
  }
  ...
  return db.transaction(async (tx) => {
    ...
    if (!attempt) throw Object.assign(new Error("Quiz attempt not found."), { code: "NOT_FOUND" });
    if (attempt.completed_at) throw Object.assign(new Error("Quiz has already been completed."), { code: "COMPLETED" });
    ...
    if (!question) throw Object.assign(new Error("Question does not belong to this quiz."), { code: "FORBIDDEN" });

    const duplicate = await tx.get(
      "SELECT id FROM quiz_answers WHERE attempt_id = $1 AND question_id = $2",
      [attemptId, questionId]
    );
    if (duplicate) throw Object.assign(new Error("Question has already been answered."), { code: "DUPLICATE" });
...
async function completeQuiz(db, userId, attemptId) {
  if (!Number.isInteger(attemptId) || attemptId <= 0) {
    throw Object.assign(new Error("Invalid quiz attempt."), { code: "VALIDATION" });
  }
...
async function result(db, userId, attemptId) {
  ...
  if (!attempt) throw Object.assign(new Error("Quiz result not found."), { code: "NOT_FOUND" });
...
module.exports = { startQuiz, answerQuestion, completeQuiz, history, result, categories, normalizeCategory, normalizeDifficulty, isAnswer };
